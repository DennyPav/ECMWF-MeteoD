...
def process_unified_venues(venues_path, datasets, run_info, s3_client, tf_instance, process_air=False):
    ...
    for city, info in venues.items():
        try:
                        # ============================================================================
            # ECMWF: TRIORARIO + ESAORARIO + GIORNALIERO
            # ============================================================================
            ...
            def daily_from_records(records):
                ...
                for d in sorted(days_map.keys()):
                    ...
                    for w in weather_list:
                        if "COPERTO" in w:
                            cloud_counter["COPERTO"] += 1
                        elif "NUVOLOSO" in w:
                            cloud_counter["NUVOLOSO"] += 1
                        elif "POCO NUVOLOSO" in w:
                            cloud_counter["POCO NUVOLOSO"] += 1
                        elif "SERENO" in w:
                            cloud_counter["SERENO"] += 1
            ...
            for d_key, h_key in sorted(tri_groups.keys()):
                ...
                mucape_avg = float(np.mean(g["mucape"]))
                most_common_dir = Counter(g["wd"]).most_common(1)[0][0]

                w3 = classify_weather(t_avg, rh_avg, ct_avg, tp_sum, wk_avg, mucape_avg, season_thresh, 3)
            ...
            for d_key, h_key in sorted(esa_groups.keys()):
                ...
                w6 = classify_weather(t_avg, rh_avg, ct_avg, tp_sum, 5.0, mucape_avg, season_thresh, 6)
